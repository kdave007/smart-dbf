...
class BatchProcessor:
    ...
    def process_all_operations(self, operations_obj, schema_type, field_id, version):
        ...
        results = {
            ...
            "updated": None,
            "deleted": None,
            "unchanged": len(operations_obj.get('unchanged', []))
        }
        
        # Process NEW records in batches
        if operations_obj['new']:
            results["new"] = self._process_in_batches(
        ...
        if operations_obj['changed']:
            results["updated"] = self._process_in_batches(
        ...
        if operations_obj['deleted']:
            results["deleted"] = self._process_in_batches(
        ...
        if operations_obj['unchanged']:
            logging.info(f" Unchanged records ({len(operations_obj['unchanged'])}) - no action needed")
    ...
    def _process_in_batches(self, records, operation_type, schema_type, field_id, version, send_method, track_method):
        ...
        logging.info(f" **************** Sending {total_records} {operation_type} records in batches of {self.batch_size}...")
        ...
        try:
            ...
            logging.error(f"Fatal error processing {operation_type} batches: {str(e)}")
            import traceback
            traceback.print_exc()
        ...
        return {
            ...
            "success_rate": round((successful_batches / total_batches * 100), 2) if total_batches > 0 else 0
        }